...
def ensure_api_key() -> None:
    """Set ANTHROPIC_API_KEY from Claude Code credentials if not already set."""
...
def execute_directives(
    ...
    for i, line in enumerate(directives):
        ...
        if action == "pane":
            ...
            print(f"  Exec: {args[:80]}...", file=sys.stderr)
...
def cmd_start(step: int = 0) -> None:
    ...
    if step > 0:
        time.sleep(1.0)  # wait for presenterm to finish loading
        for _ in range(step):
            subprocess.run(
